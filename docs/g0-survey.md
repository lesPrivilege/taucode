...
这是「构造 vs 补偿」裁定的又一实例验证：上游已将 provider-specific 语义内化为通用 message model（thinking block），消费侧（extension）无需为每个 provider 补偿——即使预设的失效方向（需要 extension 侧显式保全 reasoning_content）也未命中。

引用：`pi/packages/ai/src/providers/openai-completions.ts`（reasoning_content 映射）; `pi/packages/ai/src/types.ts:385`（AssistantMessage.content 含 ThinkingBlock）。
