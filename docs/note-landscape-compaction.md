# Note — compaction 方案 landscape 与 taucode 定位
...
**2. DeepSeek 生态缺 server-side compaction——这就是 taucode 的空档。**
...
逼近。若 DeepSeek 日后上线 server-side compaction,taucode 的 extension
...
frontier 用 server-side + LLM summary 解决「忘什么」;taucode 在
